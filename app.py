...
local_storage = LocalStorage.__new__(LocalStorage)
try:
    local_storage.__init__()
...
if "projects" not in st.session_state:
    ...
    if stored_data:
        # 1. Identify Version (If missing, it's the old versionless data)
        saved_version = stored_data.get("app_version", "1.0.0")
        
        # 2. Extract Projects
        if "projects_dict" in stored_data:
            loaded_projects = stored_data["projects_dict"]
            st.session_state.current_proj_id = stored_data.get("current_proj_id", list(loaded_projects.keys())[0])
            st.session_state.proj_counter = stored_data.get("proj_counter", 1)
        else:
            # Fallback for the very first format you used
            loaded_projects = stored_data
            st.session_state.current_proj_id = list(loaded_projects.keys())[0]
            st.session_state.proj_counter = len(loaded_projects)

        # 3. MIGRATION: 1.0.0 -> 1.1.0
        # This adds the missing 'vis_' keys needed for the Area Calculator graphic
        if saved_version < "1.1.0":
            st.toast(f"Updating ProCalc to v{APP_VERSION}...")
            
            for pid in loaded_projects:
                project_data = loaded_projects[pid].get("data", {})
                
                # Check if the specific keys for the 2D graphic exist
                if "vis_land" not in project_data:
                    # Inject default building dimensions so the graphic works immediately
                    project_data.update({
                    ...
                    loaded_projects[pid]["data"] = project_data
...
def show_area_calculator():
    ...
    with tab1:
        ...
        df_area = pd.DataFrame(st.session_state[base_key])
